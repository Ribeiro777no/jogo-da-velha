...
class MainActivity : AppCompatActivity() {
    ...
    fun buttonClick(view: View) {
        ...
        buttonSelecionado.setBackgroundResource(R.drawable.terra)
        ...
        if(!vencedor.isNullOrBlank()) {
            ...
            when (posicao) {
                0 -> {
                    binding.buttonZero.setBackgroundResource(R.drawable.lua)
                    binding.buttonZero.isEnabled = false
                }

                1 -> {
                    binding.buttonUm.setBackgroundResource(R.drawable.lua)
                    binding.buttonUm.isEnabled = false
                }

                2 -> {
                    binding.buttonDois.setBackgroundResource(R.drawable.lua)
                    binding.buttonDois.isEnabled = false
                }

                3 -> {
                    binding.buttonTres.setBackgroundResource(R.drawable.lua)
                    binding.buttonTres.isEnabled = false
                }

                4 -> {
                    binding.buttonQuatro.setBackgroundResource(R.drawable.lua)
                    binding.buttonQuatro.isEnabled = false
                }

                5 -> {
                    binding.buttonCinco.setBackgroundResource(R.drawable.lua)
                    binding.buttonCinco.isEnabled = false
                }

                6 -> {
                    binding.buttonSeis.setBackgroundResource(R.drawable.lua)
                    binding.buttonSeis.isEnabled = false
                }

                7 -> {
                    binding.buttonSete.setBackgroundResource(R.drawable.lua)
                    binding.buttonSete.isEnabled = false
                }

                8 -> {
                    binding.buttonOito.setBackgroundResource(R.drawable.lua)
